...
Terraform also materializes `.keep` placeholders in the main medallion prefixes of `data_lake`, so `bronze`, `silver`, and `gold` are visible in S3 before the first Glue run. The ETL still owns the real dataset contents.
